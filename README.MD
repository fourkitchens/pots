...
```
diff -u --color .circleci/config.yml ./vendor/ensembleiq/project_ci/config.yml
```
...
1. Install the package and copy over the template config.yml
    ...
    ```
    composer require ensembleiq/project_ci
    mkdir .circleci
    cp vendor/ensembleiq/project_ci/config.yml .circleci/config.yml
    ```
...
```
See the [Dependencies](https://github.com/ensembleiq/project_ci#dependencies) section for examples.
